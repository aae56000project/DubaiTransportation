...
% Metro Red Line - 
MetroRedLine_s = [21:47 22:48];
MetroRedLine_t = [22:48 21:47];
...
for j=1:length(br7_names)
    br7_names(j) = strcat(br7_names(j),' xr7');
end
...
for j=1:length(br8_names)
    br8_names(j) = strcat(br8_names(j),' xr8');
end
...
%R10
br10_s = [114:127 115:128];
br10_t = [115:128 114:127];
...
for j=1:length(br10_names)
    br10_names(j) = strcat(br10_names(j),' xr10');
end
...
'Khawaneej, Women’s Association y2'
'Faisal Stud y2'
'Khawaneej, Palace Masjid y2'
'Khawaneej 1 y2'
'Amardi, Boys School y2'
'Khawaneej, Health Center y2'
'Bin Bisher Farm '
...
'Awir, CMC'
'Awir, CMC Masab Bin Umair Masjid y2'
'Awir, CMC Road 4 y2'
'Awir, CMC Road 3 y2'
'Awir, Waha School y2'
'Awir, Palace y2'
'Awir, Dewa Wells Unit y2'
'Awir, CMC Road 1 y2'
'Awir, Masjid'
'Awir, Boys School'
...
for j=1:length(br11a_names)
    br11a_names(j) = strcat(br11a_names(j),' xr11a');
end


%R11b
br11b_s = [302:355 303:356];
br11b_t = [303:356 302:355];
br11b_weights = ones(1,2*(356-302));
br11b_names = ...
{
...
for j=1:length(br11b_names)
    br11b_names(j) = strcat(br11b_names(j),' xr11b');
end



%R12
br12_s = [357:397 358:398];
br12_t = [358:398 357:397];
br12_weights = ones(1,2*(398-357));
br12_names = ...
{
...
for j=1:length(br12_names)
    br12_names(j) = strcat(br12_names(j),' xr12');
end
...
for j=1:length(br13_names)
    br13_names(j) = strcat(br13_names(j),' xr13');
end
...
for j=1:length(br15_names)
    br15_names(j) = strcat(br15_names(j),' xr15');
end
...
for j=1:length(br17_names)
    br17_names(j) = strcat(br17_names(j),' xr17');
end
...
for j=1:length(br21_names)
    br21_names(j) = strcat(br21_names(j),' xr21');
end
...
for j=1:length(br22_names)
    br22_names(j) = strcat(br22_names(j),' xr22');
end
...
for j=1:length(br24_names)
    br24_names(j) = strcat(br24_names(j),' xr24');
end
...
for j=1:length(br27_names)
    br27_names(j) = strcat(br27_names(j),' xr27');
end
...
for j=1:length(br28_names)
    br28_names(j) = strcat(br28_names(j),' xr28');
end
...
for j=1:length(br31_names)
    br31_names(j) = strcat(br31_names(j),' xr31');
end
...
for j=1:length(br43_names)
    br43_names(j) = strcat(br43_names(j),' xr43');
end
...
for j=1:length(br44_names)
    br44_names(j) = strcat(br44_names(j),' xr44');
end
...
%Bus Summation
Bus_s = [br7_s br8_s br10_s br11a_s br11b_s br12_s];
Bus_t = [br7_t br8_t br10_t br11a_t br11b_t br12_t];
Bus_weights = [br7_weights br8_weights br10_weights br11a_weights br11b_weights br12_weights];
Bus_names = [br7_names 
             br8_names
             br10_names
             br11a_names
